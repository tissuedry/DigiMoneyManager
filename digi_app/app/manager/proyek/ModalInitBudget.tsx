...
import { ribuanToNumber, formatRibuan } from "./utils";

type ModalInitBudgetProps = {
    showInitBudget: Project | null;
    setShowInitBudget: (p: Project | null) => void;
    detailedProjectInfo: any;
    rabTotal: string;
    setRabTotal: (val: string) => void;
    posAnggaranList: { deskripsi: string; nominalAlokasi: string }[];
    setPosAnggaranList: React.Dispatch<React.SetStateAction<any[]>>;
    masterBudgetOptions: string[];
    masterMainOptions: any[];
    formError: string;
    submitting: boolean;
    handleInitBudget: (e: React.FormEvent) => void;
};

export default function ModalInitBudget({
    showInitBudget,
    setShowInitBudget,
    detailedProjectInfo,
    rabTotal,
                                ...
                                {posAnggaranList.map((pos, idx) => (
                                    <div key={idx} className="flex items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-150">
                                        ...
                                        >
                                            ...
                                            {/* Map data dari database masterMain */}
                                            {masterMainOptions.map((item) => (
                                                <option key={item.id} value={item.nama}>
                                                    {/* Catatan: Ganti item.nama dengan nama field/kolom tabel masterMain Anda */}
                                                    {item.nama ? item.nama.toUpperCase() : ''}
                                                </option>
                                            ))}

                                            {/* Opsi bawaan cadangan jika data tidak ada di database */}
                                            {!masterMainOptions.some(item => item.nama === "Akomodasi & Transportasi") && (
                                                <option value="Akomodasi & Transportasi">AKOMODASI & TRANSPORTASI</option>
                                            )}
                                            {!masterMainOptions.some(item => item.nama === "Konsumsi") && (
                                                <option value="Konsumsi">KONSUMSI</option>
                                            )}
                                            {!masterMainOptions.some(item => item.nama === "Perlengkapan & ATK") && (
